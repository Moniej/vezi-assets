...
Computes debt_to_equity, ebitda_margin, ebit_margin, net_margin, and
cfo_to_net_profit for every ticker/period in the real, frozen
`fsi-phase2-baseline-2026-08-01` dataset, using
src/ngxrot/fre/financial_ratios.py. Writes to the new
financial_reasoning_conclusions / financial_reasoning_conclusion_facts
tables (FSI Phase 3 schema addition) -- no extracted_facts row is ever
written or modified by this script.

  PYTHONPATH=src python scripts/fre/fsi_phase3_compute_metrics.py            # dry run
  PYTHONPATH=src python scripts/fre/fsi_phase3_compute_metrics.py --apply    # writes for real
"""
...
from ngxrot.fre.financial_ratios import list_tickers, compute_ratios_for_ticker, write_ratio_results  # noqa: E402
...
def main() -> int:
    ...
    if args.apply:
        backup_path = real_db.parent / f"ngx.sqlite.pre_fsi_phase3_metrics_backup_{date.today().isoformat()}"
    ...
    for ticker in list_tickers(con):
        results = compute_ratios_for_ticker(con, ticker)
        for r in results:
            print(f"{'[DRY RUN] ' if not args.apply else ''}ticker={r.ticker} metric={r.metric} "
                  ...
                  f"value={r.value_numeric} tier={r.confidence_tier} "
                  f"inputs={r.input_fact_ids}")
        if args.apply:
            total_written += write_ratio_results(con, results)
