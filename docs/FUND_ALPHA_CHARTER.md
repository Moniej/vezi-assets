...
| ~~Monitoring orchestration~~ — **DONE 2026-08-11.** `fre/continuous_intelligence.py`'s deterministic pipeline (Phase 18: change detection -> materiality -> alert entry) was logic-only, no persistence, no scheduler-callable form. New `monitoring_runs`/`alerts` tables (append-only, idempotent via `UNIQUE(ticker, as_of_date, prior_date)`) plus `scripts/run_continuous_intelligence.py` — an external-scheduler-callable orchestrator (no cron daemon built, per the standing "don't overengineer" instruction; this is what cron/Task Scheduler would invoke). Real bug found and fixed before this could run in any reasonable time: `process_new_information`'s own `intelligence_cache` parameter existed for exactly this but was never being passed — a single ticker cold-took ~25-84s; sharing one cache dict across a batch dropped every subsequent ticker to ~3s. Run for real against 3 tickers: generated a genuine CRITICAL alert (CBN bank-capital/LDR regulatory changes, flagged for UCAP and GTCO) and a HIGH alert (NASCON leverage increase) — not synthetic examples. 8 new regression checks against the real, persisted results. | closed | — |
| ~~Research quality/completeness exposure in the Research Workspace~~ — **DONE 2026-08-11.** `research_workspace.py` had a market-data-side quality summary (`project_quality_summary`, price data-quality flags) but nothing surfacing document/FRE-side coverage. New `document_completeness_summary()` reads from already-recorded `research_evidence` (no re-computation of the comparatively expensive `build_reasoning_context`); wired into `integrity_check()` (per-ticker missing-dimension and conflict warnings) and `export_markdown()` (new "Research Completeness" section, coverage/ceiling/missing-dimensions/conflicts table). A ticker referenced only by a price query, with no `document_context` attached, is now explicitly flagged as missing document coverage rather than silently treated as complete. `research_query.py`'s `document_context` row extended to carry `dimensions_missing` and the evidence trust-tier distribution/conflict counts (both already computed by `assess_coverage`/`evidence_ranking_summary`, previously not surfaced). Evidence quality, information coverage, and model confidence remain three distinct, never-collapsed concepts, per the standing design principle. 10 new checks, real end-to-end run against NASCON. | closed | — |
| ~~Product/API exposure~~ — **DONE 2026-08-11, CLI only — no HTTP server built.** Audit: `research_workspace.py` had a full, tested Python API but zero CLI (only usable from a Python REPL/script); the new `alerts` table had a write path but nothing to view/acknowledge it except raw SQL. Per the standing "don't introduce an API simply because APIs are fashionable" instruction and no demonstrated multi-user need, **no web server was built** — new `scripts/ngxrot_workspace.py` (create/list/show/note/market-evidence/doc-evidence/finding/hypothesis/completeness/integrity/snapshot/export/archive, matching `ngxrot_research.py`'s existing argparse convention exactly) and `scripts/ngxrot_alerts.py` (list/show/acknowledge) complete the CLI-first pattern instead. A real alert was accidentally acknowledged while smoke-testing this and had to be reverted directly — the regression test for `acknowledge` now runs against a scratch DB only, never the real `alerts` table, specifically so that mistake can't recur. 22 new checks (15 workspace CLI via subprocess against real data, 7 alerts CLI against a scratch DB). | closed | — |
