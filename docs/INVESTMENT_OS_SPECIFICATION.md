...
| Monitoring / alerting | **Orchestration BUILT 2026-08-11; scheduling still external** | `fre/continuous_intelligence.py` (Phase 18) already had the deterministic pipeline — change detection -> materiality -> alert entry, structurally refusing LOW-materiality alerts — but no persistence and nothing to call it. New `monitoring_runs`/`alerts` tables (idempotent, append-only) plus `scripts/run_continuous_intelligence.py`, callable by an external scheduler (cron/Task Scheduler — none built here, per "don't overengineer"). Real bug found before this could run at any reasonable speed: the pipeline's own `intelligence_cache` parameter, designed to amortize expensive shared computation across a batch, was never being passed — ~25-84s/ticker cold vs. ~3s warm once actually shared. Run for real: generated a genuine CRITICAL alert (CBN bank-capital/LDR regulatory changes affecting UCAP/GTCO) and a HIGH alert (NASCON leverage increase). 8 regression checks against the real persisted results. |
...
| Secondary-source (news) infrastructure | **PARTIAL — registry BUILT 2026-08-11, extraction deferred** | Prior research (`docs/STAGE10A`-`STAGE14`, 2026-08-08) already audited 5 candidate sources for an alpha hypothesis (H-019) and found Nairametrics + MarketForces Africa the only two clearing both the access-policy bar (open `robots.txt`, no AI-crawler block, verified directly) and real NGX ticker coverage — Reuters/Bloomberg/FT are policy-blocked. New `news_outlets` table (schema proposed in STAGE10A, created here) plus `sources` rows for both outlets; `evidence_ranking.assign_trust_tier` now consults it for a real tier instead of the provisional fallback. Reliability tiers (both proposed tier 3 `secondary_reputable`) are OWNER-PROPOSED defaults pending confirmation, per this platform's own "never infer an outlet's trust tier" rule. 27 real, already-staged articles (from the STAGE10 program) registered as `documents` rows across 17 real tickers — metadata only, no LLM extraction run (no `GEMINI_API_KEY` configured). Deliberately NOT done: `ngx_pulse.fetch_corporate_actions`/`fetch_events` live ingestion — both feed live Alpha Engine inputs (`corporate_actions`, `events`), same alpha-safety class as the dividend load, needs its own explicit decision. |
| Temporal / point-in-time integrity, document/FRE side | **Two real look-ahead gaps CLOSED 2026-08-11** | `documents/retrieval.py`'s `find_entity_relationships`/`find_peer_propagations` had NO date filtering at all — confirmed `entity_relationships.valid_from` genuinely spans 2020-2026 on the real database, `investment_implications.generated_at` spans 2026-07-22 to 2026-08-10. Both now accept `as_of`, wired directly into `context.build_reasoning_context` (the core reasoning engine every FRE consumer uses, not just the query bridge) — a query for a past `as_of` now genuinely differs from one for today, verified directly (was previously IDENTICAL regardless of `as_of`). `research_query.py`'s `facts` query type also fixed: `as_of` alone with no explicit `end` previously returned every fact regardless of filing date. 11 look-ahead-trap tests (`scripts/test_pit_document_side.py`), real-database-verified, mirroring `phase1_smoke_test.py`'s "prove the trap is blocked" pattern. Already-correct and NOT touched: `documents.filing_date`/`retrieved_date`/`as_of_date`, `pit_financial_memory.py`'s filing-date gating, `db.py`'s price/index bitemporal PIT. |
| Research quality/completeness exposure | **BUILT 2026-08-11** | `research_workspace.py` previously only summarized market-data quality (`project_quality_summary`); new `document_completeness_summary()` reads already-recorded `research_evidence` (no expensive re-computation) and reports per-ticker coverage score, confidence ceiling, missing dimensions, evidence trust-tier distribution, and unresolved conflicts — wired into `integrity_check()` and a new "Research Completeness" section in `export_markdown()`. A ticker with no `document_context` attached is explicitly flagged, not silently assumed complete. §46 below ("what the coverage score means") is the interpretation guidance this feature now surfaces directly inside a research project, not just in this document. |
| Product/API exposure | **BUILT 2026-08-11, CLI only** | `research_workspace.py` had a full Python API but no CLI at all; the `alerts` table (built last phase) had a write path but no way to view/acknowledge it except raw SQL. No HTTP server built — no demonstrated multi-user need, per the standing "don't introduce an API simply because APIs are fashionable" instruction. New `scripts/ngxrot_workspace.py` and `scripts/ngxrot_alerts.py` complete the CLI-first pattern `ngxrot_research.py` already established for the query layer. 22 new checks; the alert-acknowledge test runs against a scratch DB only after a real alert was accidentally acknowledged during manual testing and had to be reverted. |
